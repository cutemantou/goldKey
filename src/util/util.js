  ...
  switch (type) {
    case 'number':
      ...
      case 'object':
        ...
        return true
      case 'string':
        // null 只针对特殊情况，后端查不到数据时有的会返回null
...
/**
 * 以Blob下载文件，
 * @param {Blob} fileData Blob文件数据
 * @param {string} fileName  下载的文件名称
 * @param {string} mimeType  下载的文件mime类型,默认下载.xlsx文件
 */
export function downLoadBlob(fileData, fileName, mimeType) {
  ...
  r.onload = function () {
    try {
      ...
      let err_msg = `错误码${resData.code},错误信息:${resData.msg},`
      if (typeof resData.data === 'object') {
        if (resData.data.message && resData.data.message != null && resData.data.message !== '') {
          err_msg += resData.data.message
        } else if (resData.data.msg && resData.data.msg != null && resData.data.msg !== '') {
          err_msg += resData.data.message
        }
      } else {
        err_msg += resData.data
      }
      Message({
  ...
  r.readAsText(fileData) // FileReader的API
}
